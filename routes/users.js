...
router.get("/", async (req, res) => {
    try {
        const users = await User.find().select('username role profilePic profileDescriptor founderAffiliation isPrivate isFollowersOnly followers following followRequests settings createdAt lastSeen missionsStreak lastMissionCompleted missionsCompletedCount');
...
router.get("/find/:id", verifyToken, async (req, res) => {
    try {
        const foundUser = await User.findById(req.params.id).select('username role profilePic bio profileDescriptor founderAffiliation isPrivate isFollowersOnly followers following followRequests settings createdAt lastSeen');
...
router.get("/:id/followers", async (req, res) => {
    try {
        ...
        const followers = await User.find({ _id: { $in: user.followers } }).select('username role profilePic profileDescriptor founderAffiliation settings');
...
router.get("/:id/following", async (req, res) => {
    try {
        ...
        const following = await User.find({ _id: { $in: user.following } }).select('username role profilePic profileDescriptor founderAffiliation settings');

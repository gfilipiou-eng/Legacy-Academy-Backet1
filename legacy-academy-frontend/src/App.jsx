...
const playCyberSFX = () => {}; // Sound effects disabled
...
const StoriesBar = ({ stories, user, onAddStory, onViewStory, imgKey }) => {
    ...
    return (
        <div className="flex gap-4 overflow-x-auto no-scrollbar py-4 px-4 border-b border-white/5 bg-transparent">

            {/* CURRENT USER — Add Story */}
            <div onClick={onAddStory} className="flex flex-col items-center gap-2 cursor-pointer shrink-0 group w-[72px]">
                <div className="relative">
            ...
            {stories && stories.map((s, i) => {
                ...
                // Vibrant gradient rings per user
                const rings = [
                ...
                return (
                    <div key={s._id || i} onClick={() => onViewStory(s)} className="flex flex-col items-center gap-2 cursor-pointer shrink-0 group w-[72px]">
                        <div className="relative">
                            ...
                            {/* Profile avatar badge (bottom-left) */}
                            <div className="absolute bottom-0 left-0 w-[22px] h-[22px] rounded-full border-2 border-black overflow-hidden shadow-lg z-10 bg-neutral-900">
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 pb-4 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="relative">
                            <textarea
                                ...
                                className="relative w-full bg-black/40 border border-white/10 rounded-[1.5rem] px-5 py-4 text-[15px] text-white outline-none min-h-[120px] resize-none placeholder:text-gray-500 transition-all duration-300 custom-scrollbar font-bold break-words whitespace-pre-wrap"
                            />
                    ...
                    <div onClick={() => fileRef.current.click()} className="cursor-pointer mb-2 group mt-2">
                        {preview ? (
                            <div className="w-full min-h-[170px] sm:min-h-[220px] rounded-[1.8rem] overflow-hidden relative bg-gradient-to-br from-amber-950/20 via-slate-950/90 to-slate-900 border border-white/10 flex items-center justify-center">
                                {isVideo ? (
                            ...
                            <div className="w-full py-10 sm:py-12 border-2 border-dashed border-white/10 bg-black/30 rounded-[1.8rem] flex flex-col items-center justify-center gap-4 text-gray-500 cursor-pointer">
                                ...
                                <div className="flex flex-col items-center text-center gap-1">
                                    <span className="text-sm sm:text-base font-bold uppercase tracking-widest text-white">{t('UPLOAD_MEDIA')}</span>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 pb-4 flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                        ...
                        <textarea
                            ...
                            maxLength={300}
                            placeholder={t('DECRYPT_PH') || "Decrypt your thoughts..."}
                            className="w-full bg-black/40 border border-white/10 rounded-[1.5rem] px-5 py-4 text-[15px] text-white outline-none min-h-[120px] resize-none placeholder:text-gray-500 focus:border-[var(--gold-primary)]/50 transition-all duration-300 custom-scrollbar font-bold break-words whitespace-pre-wrap"
                        />
                    ...
                    <div onClick={() => fileRef.current.click()} className="cursor-pointer mb-2 group mt-2">
                        {preview ? (
                            <div className="w-full min-h-[170px] sm:min-h-[220px] rounded-[1.8rem] overflow-hidden relative bg-black/50 border border-white/10 flex items-center justify-center">
                                {isVideo ? (
                                    <video src={preview} className="w-full h-full object-contain max-h-[220px]" controls playsInline />
                                ) : isAudio ? (
                                    <div className="w-full p-4">
                            ...
                            <div className="w-full py-10 sm:py-12 border-2 border-dashed border-white/10 bg-black/30 rounded-[1.8rem] flex flex-col items-center justify-center gap-4 text-gray-500 cursor-pointer transition-all duration-300 hover:border-[var(--gold-primary)]/50 hover:bg-white/5 hover:text-[var(--gold-primary)]">
                                <div className="w-16 h-16 sm:w-18 sm:h-18 rounded-full bg-white/5 flex items-center justify-center border border-white/10">
                                    <Icons.Upload className="w-7 h-7 sm:w-8 sm:h-8 text-[var(--gold-primary)]" />
                                </div>
                                <div className="flex flex-col items-center text-center gap-1">
    ...
    useEffect(() => {
        ...
        const onMessageRecv = (msg) => {
            ...
            if (user && String(msg.recipient) === String(user._id) && String(msg.sender) !== String(user._id)) {
                ...
                const messageText = `${t('NOTIF_MESSAGE', 'New message from')} ${senderName}`;
                // Trigger browser notification
                if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
                    try {
                    ...
                    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-2xl">
                        <div className="relative bg-black/40 backdrop-blur-[80px] border border-white/10 rounded-[32px] md:rounded-[36px] max-w-[420px] w-full max-h-[95dvh] overflow-y-auto no-scrollbar">
                            ...
                            <div className="relative p-4 sm:p-8 text-center flex flex-col items-center">
                                ...
                                <button onClick={() => window.location.href = "https://buy.stripe.com/aFaaEX81B2Fs1gI36Y6Na07"} className="mt-2 w-full relative group overflow-hidden rounded-2xl py-4 flex items-center justify-center gap-3 font-black text-sm uppercase tracking-[0.2em] disabled:opacity-40 hover:opacity-90 transition-all duration-300 bg-gradient-to-r from-[#D4AF37] to-[#AA771C] text-black border border-white/20 shadow-none cursor-pointer mb-3 sm:mb-6">
                                    <div className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        ...
                        <div id="zoomable-content" className="pt-0 sm:pt-4 max-w-2xl sm:max-w-xl md:max-w-2xl mx-auto pb-[158px]">
                            {activeTab === 'exchange' ? (
                                ...
                                <>
                                    ...
                                    {activeTab !== 'search' && activeTab !== 'cartels' && <StoriesBar stories={stories} user={user} imgKey={imgKey} key={imgKey || 'stories'} onAddStory={() => { setCreateModeStory(true); setIsCreateOpen(true); }} onViewStory={(s) => setSelectedPost(s)} />}
                                ...
                                <div className="flex flex-col items-start text-left max-w-[calc(100%-60px)]">
                                    <div className="font-bold text-white text-base flex flex-wrap items-center gap-1.5 leading-none w-full">
                                        ...
                                        {shareModalPost.author?.profileDescriptor && PROFILE_DESCRIPTOR_MAP[shareModalPost.author.profileDescriptor] && (
                                            <div className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 backdrop-blur-xl text-[9px] font-black uppercase tracking-wider shrink-0 ${getDescriptorAccentClass(shareModalPost.author.profileDescriptor, shareModalPost.author?.role).replace(/rounded-none/g, "")}`}>

        ...
        <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, x: -10 }} className={`flex gap-3 items-start relative mb-5 ${isCommentAuthor ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex-1 min-w-0 flex flex-col ${isCommentAuthor ? 'items-end' : 'items-start'}`}>
                ...
                <div className="flex flex-wrap gap-2 mt-2 px-1 items-center">
                    ...
                    {canEdit && !isEditing && (
                        <button type="button" onClick={() => setIsEditing(true)} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[9px] font-black uppercase hover:bg-blue-500/20 transition-colors" title={t('EDIT')}>
                            <Icons.Edit className="w-3 h-3" /> {t('EDIT')}
                        </button>
                    )}
                    {canDelete && (
                        <button type="button" onClick={() => onDelete?.(post._id, comment._id)} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-[9px] font-black uppercase hover:bg-red-500/20 transition-colors" title={t('DELETE')}>

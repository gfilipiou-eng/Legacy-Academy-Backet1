    ...
    return (
        <div className="flex items-center gap-3 bg-black/40 backdrop-blur-xl border border-white/5 hover:border-[var(--gold-primary)]/30 rounded-2xl p-2 px-3 w-full max-w-[280px] shadow-2xl transition-all group/voice" onClick={(e) => e.stopPropagation()}>
            <button
                type="button"
                onClick={togglePlay}
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shrink-0 transition-all shadow-[0_0_15px_rgba(var(--gold-primary-rgb),0.3)] group-hover/voice:shadow-[0_0_20px_rgba(var(--gold-primary-rgb),0.5)] active:scale-95 ${isPlaying ? 'bg-white text-black' : 'bg-[var(--gold-primary)] text-black'}`}
            >

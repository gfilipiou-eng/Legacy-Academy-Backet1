...
// Update handlePublish to take isDraft
code = code.replace(/const handlePublish = async \(\) => \{/g, 'const handlePublish = async (isDraft = false) => {');
code = code.replace(/const updatedConfig = \{ \.\.\.config, lastUpdated: new Date\(\) \};/g, 'const updatedConfig = { ...config, lastUpdated: new Date(), isDraft };');

// Update the header buttons to include Save Draft
const newButtons = `
...
code = code.replace(/<button[\s\S]*?onClick=\{handlePublish\}[\s\S]*?<\/button>/, newButtons);

// Fix all inputs to have || ''
...
console.log('WebsiteBuilder patched');

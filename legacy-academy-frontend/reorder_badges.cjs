...
// 1. Post Card Header (Lines ~2696)
code = code.replace(
    /<VerifiedBadge isFounder=\{isFounder\} isUser=\{!isFounder && \(author\?\.settings\?\.showBadge !== false\)\} className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 flex-shrink-0" user=\{author\} hideFootball=\{true\} \/>\s*\{author\?\.profileDescriptor && PROFILE_DESCRIPTOR_MAP\[author\.profileDescriptor\] && \([\s\S]*?<\span className="text-\[9px\] font-black uppercase tracking-\[0\.12em\]">\{t\(`DESC_\$\{author\.profileDescriptor\.toUpperCase\(\)\}`, PROFILE_DESCRIPTOR_MAP\[author\.profileDescriptor\]\.label\)\}<\/span>\s*<\/div>\s*\)\}\s*\{getActiveStreak\(author\) > 0 && <span className="text-orange-500 font-bold text-\[11px\] sm:text-xs shrink-0 flex items-center"><Icons\.Streak className="inline-block w-\[1\.1em\] h-\[1\.1em\] -mt-1" \/>\{getActiveStreak\(author\)\}<\/span>\}/,
...
// 2. Share Modal Post (Lines ~11853)
code = code.replace(
    /<VerifiedBadge isFounder=\{shareModalPost\.author\?\.role === 'Founder'\} isUser=\{shareModalPost\.author\?\.role !== 'Founder'\} className="w-4 h-4 shrink-0" user=\{shareModalPost\.author\} \/>\s*\{shareModalPost\.author\?\.profileDescriptor && PROFILE_DESCRIPTOR_MAP\[shareModalPost\.author\.profileDescriptor\] && \([\s\S]*?<\span className="text-\[9px\] font-black uppercase tracking-\[0\.12em\]">\{t\(`DESC_\$\{shareModalPost\.author\.profileDescriptor\.toUpperCase\(\)\}`, PROFILE_DESCRIPTOR_MAP\[shareModalPost\.author\.profileDescriptor\]\.label\)\}<\/span>\s*<\/div>\s*\)\}\s*\{getActiveStreak\(shareModalPost\?\.author\) > 0 && <span className="text-orange-500 font-bold text-xs shrink-0 flex items-center"><Icons\.Streak className="inline-block w-\[1\.1em\] h-\[1\.1em\] -mt-1" \/>\{getActiveStreak\(shareModalPost\?\.author\)\}\{isTopStreak\(shareModalPost\?\.author\) && <span className="ml-1\.5 px-1\.5 py-0\.5 bg-gradient-to-r from-orange-400 to-red-500 text-black text-\[9px\] font-black uppercase rounded-sm shadow-md tracking-widest leading-none align-middle inline-flex items-center gap-0\.5"><Icons\.TrendingUp className="w-2\.5 h-2\.5" \/> TOP<\/span>\}<\/span>\}/,
...
// 3. Share Modal Profile (Lines ~11951)
code = code.replace(
...
// 4. Public Profile Modal Header (Lines ~8263)
code = code.replace(
...
// 5. Global Search Users List (Lines ~11530)
code = code.replace(
...
console.log('Successfully reordered badges in App.jsx');
